...
IMMATERIAL_TERMS = {
    ...
    "postal ballot", "scrutinizer report", "record date", "newspaper publication",
    "closure of trading window", "analyst/investor meet", "investor meet", "conference call",
}
...
def materiality_score(title: str, text: str, market_cap_crore: float | None = None) -> int:
    ...
    for term in MATERIAL_TERMS:
        if term in haystack:
            score += 2
    if any(term in haystack for term in ("rbi", "sebi", "government", "ministry", "inflation", "repo", "sensex", "nifty")):
        score += 3
    if any(word in haystack for word in ("revenue", "ebitda", "pat", "profit", "loss")):
        score += 3
    if any(term in haystack for term in ("rights issue", "preferential", "allotment", "fund raise", "credit rating", "default")):
        score += 2
    return min(score, 10)


def is_fresh(published_at: datetime, freshness_minutes: int) -> bool:
    now = datetime.now(timezone.utc)
    return published_at >= now - timedelta(minutes=freshness_minutes)
