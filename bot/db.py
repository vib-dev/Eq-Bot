...
class Database:
    ...
    async def ranked_latest_events(self, symbol: str, limit: int = 5):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            return await db.execute_fetchall(
                """
                SELECT *,
                    CASE
                        WHEN lower(title) LIKE '%results%' THEN 5
                        WHEN lower(title) LIKE '%board meeting%' THEN 5
                        WHEN lower(title) LIKE '%financial results%' THEN 5
                        WHEN lower(title) LIKE '%earnings%' THEN 5
                        WHEN lower(title) LIKE '%acquisition%' THEN 4
                        WHEN lower(title) LIKE '%order%' THEN 4
                        WHEN lower(title) LIKE '%contract%' THEN 4
                        WHEN lower(title) LIKE '%guidance%' THEN 4
                        WHEN category = 'announcement' THEN 3
                        ELSE 1
                    END AS priority_rank
                FROM events
                WHERE symbol = ?
                ORDER BY priority_rank DESC, materiality_score DESC, published_at DESC
                LIMIT ?
                """,
                (canonical_symbol(symbol), limit),
            )
    ...
    async def top_for_user_since(self, chat_id: int, since_iso: str, limit: int = 20):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            symbols = await self.get_user_symbols(chat_id)
            if not symbols:
                return []
            placeholders = ",".join("?" for _ in symbols)
            params: list[object] = [since_iso, *symbols, limit]
            return await db.execute_fetchall(
                f"""
                SELECT * FROM events
                WHERE published_at >= ?
                  AND symbol IN ({placeholders})
                  AND category IN ('announcement', 'news')
                ORDER BY materiality_score DESC, published_at DESC
                LIMIT ?
                """,
                tuple(params),
    ...
    async def market_briefing_since(self, since_iso: str, limit: int = 25):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            return await db.execute_fetchall(
                """
                ...
                FROM events
                WHERE published_at >= ?
                  AND materiality_score >= 3
                ORDER BY
                    CASE
                        WHEN category = 'macro' THEN 5
                        WHEN lower(title) LIKE '%results%' THEN 4
                        WHEN lower(title) LIKE '%board meeting%' THEN 4
                        WHEN lower(title) LIKE '%order%' THEN 3
                        WHEN lower(title) LIKE '%contract%' THEN 3
                        ELSE 2
                    END DESC,
